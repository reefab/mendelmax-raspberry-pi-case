...
GPIOHOLE        = 0;        // GPIO opening in :  1=bottom   2=top
...
gpioslot = false; // cutout for GPIO pins on top
...
if (DRAWtotal == 1) {
    translate(v = [ -box_w/2, -box_l / 2, 0]) {
        ...
        if (DRAWbottom) rotate([0,0,90]) draw_case(1,0);
...
// draw each one on there one location
    ...
    if (DRAWbottom) {
        translate(v = [ 25, 5, 0]) draw_case(1,0);
    }
    if (DRAWtop)    {
...
module put_pilogo(w,h,mx,mh,mt) {
  // put_pi_logo(w,h,mx,mh,mt);
  // put_dmt_logo(w,h,mx,mh,mt);
  // put_ff_logo(w,h,mx,mh,mt);
  put_nis_logo(w,h,mx,mh,mt);
  // put_text1_logo(w,h,mx,mh,mt);
  // put_text2_logo(w,h,mx,mh,mt);
}

module put_pi_logo(w,h,mx,mh,mt) {
...
module put_nis_logo(w,h,mx,mh,mt) {
    fn="nischi.dxf";
    rotate([0,0,70]) translate([22,-12,0]) 
      linear_extrude(height=mt) import(file=fn, scale=1.10);
}

module put_dmt_logo(w,h,mx,mh,mt) {
    fn="dmt_logo_r13.dxf";
    rotate([0,0,70]) translate([15,-10,0]) linear_extrude(height=mt) import(file=fn, scale=.55);
}

module put_text1_logo(w,h,mx,mh,mt) {
  rotate([0,0,90]) translate([47,-35,0])
    write("01",t=mt*2,h=17,font="orbitron.dxf", space=1.10, center=true);
}
module put_text2_logo(w,h,mx,mh,mt) {
  rotate([0,0,90]) translate([47,-35,0])
    write("BerryPlay",t=mt*2,h=9,font="orbitron.dxf", space=1.10, center=true);
}


module put_ff_logo(w,h,mx,mh,mt) {
    fn="fridafred.dxf";
    // calculate the scale and which offset to use
    scx=(w- (mx*2)) / 100.0;
    ofsy= (h - mh) - (205.0*scx);

    translate(v=[mx+toplogopc+20,ofsy-toplogotm+70,0]) linear_extrude(height=mt) import(file=fn, scale=scx);
}
...
translate(v=[Cgpio_x,Cgpio_y,0]) cube([Cgpio_w,Cgpio_d,Cgpio_h],center=false);
...
module make_connholes() {
  ...
  if (gpioslot == true) {
      translate(v=[box_w-box_thickness-Cgpio_w,box_l1+Cgpio_y,box_h-box_thickness-3]) {

        cube([Cgpio_w,Cgpio_d,box_thickness*2+2]);
    }
		translate(v=[box_w-box_thickness-Cgpio_w,box_l1+Cgpio_y+1,box_h-box_thickness-13]) {

        cube([Cgpio_w,Cgpio_d,box_thickness*2+1]);
    }
}
}
            ...
            color(CASEcolor) {
                ...
                if (top != 0) {
                    ...
                    translate(v = [box_w1, box_l1, pcb_c+0.4]) {
                        cube([stud_w, 15, box_h-pcb_c-1], center = false);
                    }
                    translate(v = [box_w1, box_l2-32, pcb_c+0.4]) {
     	                   cube([stud_w, 32, box_h-pcb_c-1], center = false) ;
					 }
                   translate(v = [box_w1, box_l1+Chdmi_y+Chdmi_d+1.5, pcb_c+0.4]) {
                    ...
                    translate(v = [box_w2-stud_w, box_l1, pcb_c+0.4]) {
                        cube([stud_w,12, box_h-pcb_c-1], center = false);
                    }
                    translate(v = [box_w2-stud_w, box_l1+Crca_y+Crca_d+1, pcb_c+0.4]) {
                        cube([stud_w, 5, box_h-pcb_c-1], center = false);
                    }
                }
            } // color
        } // end union
